...
def inference(config):
    with open('./track_model_contra/fc8.prototxt', 'w') as f:
        f.write(str(track_model_contra.generate_fc8('val', config)))
    with open('./track_model_contra/scores.prototxt', 'w') as f:
        f.write(str(track_model_contra.generate_scores('val', config)))
    ...
    fc8_net = caffe.Net('./track_model_contra/fc8.prototxt',
                        config.pretrained_model,
                        caffe.TEST)

    scores_net = caffe.Net('./track_model_contra/scores.prototxt',
                           config.pretrained_model,
                           caffe.TEST)
    ...
    for n_im in tqdm(range(num_im)):
        ...
        imcrop_val[:num_proposal, ...] = im_processing.crop_bboxes_subtract_mean(
            im, bbox_proposals, config.input_H, track_model_contra.channel_mean)
