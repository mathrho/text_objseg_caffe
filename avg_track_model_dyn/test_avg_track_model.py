...
import avg_track_model_dyn_sigmoid as track_model
import test_config
...
def inference(config):
    with open('./avg_track_model_dyn/fc8.prototxt', 'w') as f:
        f.write(str(track_model.generate_fc8('val', config)))
    with open('./avg_track_model_dyn/scores.prototxt', 'w') as f:
    ...
    fc8_net = caffe.Net('./avg_track_model_dyn/fc8.prototxt',
                        config.pretrained_model,
                        caffe.TEST)

    scores_net = caffe.Net('./avg_track_model_dyn/scores.prototxt',
                           config.pretrained_model,
                           caffe.TEST)
