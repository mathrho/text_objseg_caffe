...
import avg_lang_seg_low_res_model as seg_model
from util.processing_tools import *
import train_config


def train(config):
    with open('./avg_lang_seg_low_res_model/proto_train.prototxt', 'w') as f:
        f.write(str(seg_model.generate_model('train', config)))

    caffe.set_device(config.gpu_id)
    caffe.set_mode_gpu()

    solver = caffe.get_solver('./avg_lang_seg_low_res_model/solver.prototxt')
