...
d = dir('../results/ILSVRC/results_lang_seg_sigmoid_thresh0.3/');
...
for vi = 1:numel(videos)
    ...
    %for qi = query_ids
    f = dir(['../results/ILSVRC/results_lang_seg_sigmoid_thresh0.3/' video  '/*.jpg']);
    frames = {f(:).name}';
    ...
    for fr = 1:numel(frames)
        ...
        curHeatMapFile = ['../results/ILSVRC/results_lang_seg_sigmoid_thresh0.3/' video '/' im_name '.jpg'];
        curImgFile = ['/home/zhenyang/Workspace/data/ImageNetTracker/' video_name '/img/' im_name '.jpg'];
        curBBoxFile = ['../results/ILSVRC/results_lang_seg_sigmoid_thresh0.3/' video '/' im_name '.txt'];
