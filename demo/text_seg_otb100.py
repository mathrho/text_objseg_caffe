...
for videofile in videofiles:
    ...
    for fi in range(start_frame_id, 1+start_frame_id):
        ...
        score_thresh = np.amax(upscores) * 0.8
        prediction = im_processing.resize_and_crop(upscores>score_thresh, *im.shape[:2]).astype(np.bool)
        print( str(np.sum(prediction)) )

        # save the results
        if not os.path.exists('../results/OTB100/results_lang_seg_sigmoid_thresh0.8/'+video):
            os.makedirs('../results/OTB100/results_lang_seg_sigmoid_thresh0.8/'+video)
        if video == 'Board':
            filename = '../results/OTB100/results_lang_seg_sigmoid_thresh0.8/'+video+'/%05d.jpg' % (fi,)
        else:
            filename = '../results/OTB100/results_lang_seg_sigmoid_thresh0.8/'+video+'/%04d.jpg' % (fi,)
