...
frame_starts = []
frame_ends = []
with open('../ILSVRC/test.txt') as lines:
    for line in lines:
        tokens = line.rstrip().split(' ')
        videofiles.append(tokens[0])
        frame_starts.append(int(tokens[1]))
        frame_ends.append(int(tokens[2]))
...
for v, videofile in enumerate(videofiles):
    ...
    # First, load queries
    query_file = '../ILSVRC/ImageNetTracker100Entities_working/' + video + '.xml'
    ...
    # Second, get gt box
    gt_file = '/home/zhenyang/Workspace/data/ImageNetTracker/' + video + '/groundtruth_rect.txt'
    try:
    ...
    for q in range(num_queries):
        ...
        for fi in range(start_frame_id, num_frames+start_frame_id):
            im_file = '/home/zhenyang/Workspace/data/ImageNetTracker/' + video + '/img/%06d.jpg' % (fi,)            
            ...
            score_thresh = np.amax(upscores) * 0.6
            ...
            if not os.path.exists('../results/ILSVRC/results_lang_seg_sigmoid_thresh0.6/'+video_prefix):
                os.makedirs('../results/ILSVRC/results_lang_seg_sigmoid_thresh0.6/'+video_prefix)
            filename = '../results/ILSVRC/results_lang_seg_sigmoid_thresh0.6/'+video_prefix+'/%06d.jpg' % (fi,)
