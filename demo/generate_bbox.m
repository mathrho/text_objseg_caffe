...
d = dir('../results/OTB100/results_lang_seg_sigmoid_thresh0.6/');
...
for vi = 1:numel(videos)
    video = videos{vi}
    f = dir(['../results/OTB100/results_lang_seg_sigmoid_thresh0.6/' video '/*.jpg']);
    frames = {f(:).name}';
    frames(ismember(frames,{'.','..'})) = [];

    for fr = 1:numel(frames)
        im_name = frames{fr}(1:end-4);
        curHeatMapFile = ['../results/OTB100/results_lang_seg_sigmoid_thresh0.6/' video '/' im_name '.jpg'];
        curImgFile = ['/home/zhenyang/Workspace/data/OTB-100-othervideos/' video '/img/' im_name '.jpg'];
        curBBoxFile = ['../results/OTB100/results_lang_seg_sigmoid_thresh0.6/' video '/' im_name '.txt'];
